...
def process_files(input_folder_path, output_folder_path):
    ...
    for file, df in dataframes.items():
        if 'plaid_accounts' in file:
            ...
            dataframes[file] = remove_duplicates(df, ['account_id'])
        elif 'plaid_liabilities_credit' in file:
            logger.info(f'Removing duplicates from {file}')
            dataframes[file] = remove_duplicates(df, ['account_id'])
        elif 'plaid_liabilities_credit_apr' in file:
            logger.info(f'Removing duplicates from {file}')
            dataframes[file] = remove_duplicates(df, ['account_id', 'apr_type'])
        elif 'plaid_transactions' in file:
            logger.info(f'Removing duplicates from {file}')
            dataframes[file] = remove_duplicates(df, ['transaction_id'])
        elif 'plaid_transaction_counterparties' in file:
            logger.info(f'Removing duplicates from {file}')
            dataframes[file] = remove_duplicates(df, ['transaction_id', 'entity_id'])
        elif 'categories' in file:
            logger.info(f'Removing duplicates from {file}')
            dataframes[file] = remove_duplicates(df, ['category_id'])
        elif 'asset_report' in file:
            logger.info(f'Removing duplicates from {file}')
            dataframes[file] = remove_duplicates(df, ['asset_report_id'])
        elif 'asset_item' in file:
            logger.info(f'Removing duplicates from {file}')
            dataframes[file] = remove_duplicates(df, ['item_id'])
            ...
            logger.info(f'Removing duplicates from {file}')
            dataframes[file] = remove_duplicates(df, ['account_id', 'date'])
        elif 'mbna_accounts' in file:
            logger.info(f'Removing duplicates from {file}')
            dataframes[file] = remove_duplicates(df, ['account_number'])
        elif 'mbna_transactions' in file:
            logger.info(f'Removing duplicates from {file}')
            dataframes[file] = remove_duplicates(df, ['account_id', 'transaction_id'])
